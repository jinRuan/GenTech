...
var loadThreads = function (classId) {
  ...
  var setClassName = function (className) {
    var classHTML = "<div class='classHTML'>"+className+"</div>"
  ...
  queryClass.first().then(function (aClass) {
    var threadPromises = [];
    ...
    var className = aClass.get("name");
    setClassName(className);

    return threadPromises;
  }).then(function () {
    var threadPromises = arguments[0];
    Parse.Promise.when(threadPromises).then(function(){
